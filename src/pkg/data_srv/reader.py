...
class TiingoReader:
    ...
    def __init__(self, ctx):
        ...
        self.api_token = "75a0aa12dd1e297d613cfe92ecfe96dab95a4589"
        self.ctx = ctx
    ...
    def data_reader(self):
        ...
        #     if DEBUG: logger.debug(f"dict_list: {dict_list}")
        data_list = ['EEM', 'IWM']
        for i, symbol in enumerate(data_list):
            dict_list = fetch_ohlc[i]
            if DEBUG: logger.debug(f"_fetch_ohlc_price_data(symbol={symbol}) -> {dict_list}")
            parsed_data = self._parce_price_data(dict_list=dict_list)
            self._create_dataframe(parsed_data=parsed_data)

    def _create_dataframe(self, parsed_data):
        """"""
        df = pd.DataFrame(
            data=parsed_data,
            columns=parsed_data,
            # index=parsed_data['date']
        )
        if DEBUG: logger.debug(f"_create_dataframe(self, parsed_data) -> {df}")

    def _parce_price_data(self, dict_list):
        """"""
        data = dict()
        ohlc = list()
        for item in dict_list:
            ...
            ohlc.append(data)
        if DEBUG: logger.debug(f"_parce_price_data(self, dict_list) -> {data}")
        return data
...
if __name__ == '__main__':
    for item in fetch_ohlc:
        logger.debug(f"reader.py\n{item}")
